...
public class UserDAO {
	...
	public int inset(String username, String birthyear, String addr, String mobile) {
		...
		try {
			if (con != null) {
				// 4. sql 구문 생성
				String sql = "insert into memberTBL values(userTBL_seq.nextval,?,?,?,?)";
	...
	public List<UserVO> select() {
		...
		try {
			if (con != null) {
				// 4. sql 구문 생성
				String sql = "select * from memberTBL order by no desc";
	...
	public int updateUser(String addr, String mobile, int no) {
		...
		try {
			if (con != null) {
				String sql = "update memberTBL set addr = ?, mobile = ? where no = ?";
				pstmt = con.prepareStatement(sql);
